...
class Loss(nn.Module):
    ...
    def forward(self, outputs, **kwargs):
        ...
        for name in loss_names:
            ...
            # print(loss_dict[name].shape)
            # if name in ['CenterID', 'JointID']:
            #     loss_dict[name] = loss_dict[name] * eval('args().{}_weight'.format(name))
            # else:
            #     loss_dict[name] = loss_dict[name].mean() * eval('args().{}_weight'.format(name))
    ...
    def _calc_detection_loss(self, outputs, meta_data):
        ...
        if 'joint_maps' in outputs:
            ...
            detect_loss_dict['JointMap'] = loss_joint/24


        detect_loss_dict.update({'CenterID': 0})
        detect_loss_dict.update({'JointID': 0})
        if 'centerid_loss' in outputs:
            detect_loss_dict['CenterID'] = outputs['centerid_loss']
        if 'jointid_loss' in outputs:
            detect_loss_dict['JointID'] = outputs['jointid_loss']
        ######################################################################################################
