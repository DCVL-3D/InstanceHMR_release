...
class Base(nn.Module):
    ...
    def matching_forward(self, meta_data, **cfg):
        if args().model_precision=='fp16':
            with autocast():
                outputs = self.feed_forward(meta_data, train_flag=True)
    ...
    def parsing_forward(self, meta_data, **cfg):
        if args().model_precision=='fp16':
            with autocast():
                outputs = self.feed_forward(meta_data, train_flag=False)
    ...
    def feed_forward(self, meta_data, train_flag):
        x = self.backbone(meta_data['image'].contiguous().cuda())
        # outputs = self.head_forward(x, meta_data['kp_coords'], meta_data['person_centers'], meta_data['image'], meta_data['heatmap'])
        outputs = self.head_forward(x, meta_data['kp_coords'], meta_data['person_centers'], train_flag)
        return outputs
