    ...
    def save_rendered_img(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, save2html=False, **kwargs):
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'mesh' and detection_flag:
                rendered_imgs = self.show_verts_on_imgs(outputs, meta_data, (used_org_inds, per_img_inds, img_inds_org), \
                    org_imgs, img_names=img_names, put_org='put_org' in vis_cfg['settings'], plot_dict=plot_dict, save2html=save2html)
                # print("outputs['meta_data']['image'].shape:", outputs['meta_data']['image'].shape)  # 1, 512, 512, 3
                # print("rendered_imgs.shape:", rendered_imgs.shape)  # 1, 512, 512, 3
                # print("outputs['joint_maps'].shape:", outputs['joint_maps'].shape)  # 1, 21, 64, 64
                # print("outputs['meta_data']['heatmap'].shape:", outputs['meta_data']['heatmap'].shape)  # 1, 21, 64, 64

                # print(meta_data['imgpath'])
                index = int(os.path.basename(meta_data['imgpath'][0]).split('.')[0])
                # cv2.imwrite("path/{0:06d}_input.png".format(index), cv2.cvtColor(outputs['meta_data']['image'][0].cpu().numpy(), cv2.COLOR_RGB2BGR))
                cv2.imwrite("path/{0:06d}_mesh.png".format(index), cv2.cvtColor(rendered_imgs[0], cv2.COLOR_RGB2BGR))

    def save_rendered_img_crowdpose(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, save2html=False, **kwargs):
        vis_cfg = dict(default_cfg, **vis_cfg)
        if vis_cfg['save_dir'] is None:
            vis_cfg['save_dir'] = self.result_img_dir
        os.makedirs(vis_cfg['save_dir'], exist_ok=True)
...
#################################################################################################################################
    def save_skeleton_img(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, save2html=False, **kwargs):
        vis_cfg = dict(default_cfg, **vis_cfg)
        # if vis_cfg['save_dir'] is None:
        #     vis_cfg['save_dir'] = self.result_img_dir
        # os.makedirs(vis_cfg['save_dir'], exist_ok=True)
                ...
                for img_id, inds_list in enumerate(per_img_inds):
                    ...
                    try:
                        for kp2d_vis in outputs['pj2d'][inds_list]:
                            if len(kp2d_vis)>0:
                                kp2d_vis = ((kp2d_vis+1)/2 * org_imgs.shape[1])
                                org_img = draw_skeleton_nocircle(org_img, kp2d_vis, bones=constants.ochuman_connMat, cm=constants.cm_ochuman)
                                # org_img = draw_skeleton(org_img, kp2d_vis, bones=constants.All54_connMat, cm=constants.cm_All54)
                    except Exception as error:
                        print(error, ' reported while drawing 2D pose')
                    # kp_imgs.append(org_img)
                    ...
                    index = int(os.path.basename(meta_data['imgpath'][0]).split('.')[0])
                    cv2.imwrite("path/{0:06d}_input.png".format(index), cv2.cvtColor(outputs['meta_data']['image'][0].cpu().numpy(), cv2.COLOR_RGB2BGR))
                    cv2.imwrite("path/{0:06d}_skeleton.png".format(index), cv2.cvtColor(org_img, cv2.COLOR_RGB2BGR))

    def save_skeleton_img_crowdpose(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, save2html=False, **kwargs):
        vis_cfg = dict(default_cfg, **vis_cfg)
        # if vis_cfg['save_dir'] is None:
        #     vis_cfg['save_dir'] = self.result_img_dir
        # os.makedirs(vis_cfg['save_dir'], exist_ok=True)
                ...
                for img_id, inds_list in enumerate(per_img_inds):
                    ...
                    try:
                        for kp2d_vis in outputs['pj2d'][inds_list]:
                            if len(kp2d_vis)>0:
                                kp2d_vis = ((kp2d_vis+1)/2 * org_imgs.shape[1])
                                org_img = draw_skeleton_nocircle(org_img, kp2d_vis, bones=constants.crowdpose_connMat, cm=constants.cm_body14)
                                # org_img = draw_skeleton(org_img, kp2d_vis, bones=constants.All54_connMat, cm=constants.cm_All54)
                    except Exception as error:
                        print(error, ' reported while drawing 2D pose')
                    # kp_imgs.append(org_img)
...
def draw_skeleton_nocircle(image, pts, bones=None, cm=None, label_kp_order=False,r=3):
    for i,pt in enumerate(pts):
        if len(pt)>1:
            if pt[0]>0 and pt[1]>0:
                # image = cv2.circle(image,(int(pt[0]), int(pt[1])),r,(255,0,0),-1)
