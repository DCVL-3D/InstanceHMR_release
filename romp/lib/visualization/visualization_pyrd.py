...
class Visualizer(object):
    def __init__(self, resolution=(512,512), result_img_dir = None, with_renderer=False):
        ...
        # self.smpl_face = torch.load('/home/dcvl/MK/ROMP_Exp07_2/model_data/smpl_models/smil_packed_info.pth')['f']
        self.smpl_face = torch.load('/home/dev2/Drive_C/MK/IdHMR/Ab02/model_data/smpl_models/smpl_packed_info.pth')['f']

        self.with_renderer = with_renderer
        if with_renderer:
            self.perps_proj = args().perspective_proj
            # self.perps_proj = True
            ...
            self.renderer = get_renderer(resolution=self.resolution, perps=self.perps_proj)
    ...
    def visualize_renderer_verts_list(self, verts_list, faces_list=None, images=None, bird_view=False, auto_cam=False, cam_params=None,\
                                            colors=torch.Tensor([.9, .9, .8]), trans=None, thresh=0., org_res=None, cam=None):
        ...
        for ind, (verts, faces) in enumerate(zip(verts_list, faces_list)):
            ...
            # for n in reversed(list(range(num_people))):
                rendered_img = renderer(verts[n], faces[n], merge_meshes=False, org_res=org_res, cam=cam[ind][n])
                # rendered_imgs.append(rendered_img)
                rendered_imgs = np.array(rendered_img)
                rendered_imgs = rendered_imgs[np.newaxis, :]
                ...
                if images is not None:
                    ...
                    # print("############## rendered_img.shape: ", rendered_img.shape)
                    # print("############## images.shape: ", images.shape)
                    # print("############## valid_mask.shape: ", valid_mask.shape)
                    images = rendered_imgs * valid_mask * visible_weight + images * valid_mask * (1-visible_weight) + (1 - valid_mask) * images
        ...
        #         # verts[:,:,2] -= 1.
        #         rendered_img = renderer(verts, faces, colors=color, merge_meshes=True, camera_pose=cam_params)
        #     else:
        #         # verts[:,:,2] -= 1.
        #         # print("#"*50)
        #         # print(trans[ind])   # 2,3 tensor
        #         # print(trans[ind].shape) # 2,3
        #         # print("####### verts.shape: ", verts.shape)
        #         # print("####### faces.shape: ", faces.shape)
        #         # print("####### cam: ", cam)
        #         # print("####### ind: ", ind)

        #         rendered_img = renderer(verts, faces, colors=color, merge_meshes=False, camera_pose=cam_params, org_res=org_res, cam=cam[ind])
        #         rendered_img = determine_rendering_order(rendered_img)
        #     # print("################ rendered_img.shape: ", rendered_img.shape)  # 1, 512, 512, 4
        #     rendered_imgs.append(rendered_img[0])
        #     # rendered_imgs.append(rendered_img)
        # rendered_imgs = np.array(rendered_imgs)
        # #print("#"*10, rendered_imgs.shape)
        # #rendered_imgs = torch.from_numpy(np.array(rendered_imgs))
    ...
    def show_verts_on_imgs(self, outputs, meta_data, all_inds, org_imgs, mesh_colors=None,img_names=None, put_org=True, plot_dict=None, save2html=True, drop_texts=False):
        ...
        # print("#"*50)
        # print("verts")
        # print([outputs['verts'][inds].detach() for inds in per_img_inds])
        # print("trans")
        # print([outputs['cam_trans'][inds].detach() for inds in per_img_inds])
        # print("verts_camed")
        # print([outputs['verts_camed'][inds].detach() for inds in per_img_inds])
        if self.with_renderer:
            org_img = cv2.imread(img_names[0])
            org_res=org_img.shape[0:2]
            self.perps_proj = args().perspective_proj
            # self.perps_proj = True
            # T= None if self.perps_proj else torch.Tensor([[0.,0.,100]])                
            # self.renderer = get_renderer(resolution=self.resolution, perps=self.perps_proj, T=T)
            self.renderer = get_renderer(resolution=org_res, perps=self.perps_proj, height=org_res[0], width=org_res[1])
        ...
        if self.perps_proj:
            ...
            # print("############### img_inds_org: ", img_inds_org)
            # print("############### img_names[img_inds_org]: ", img_names[img_inds_org])
            # org_img = cv2.imread(img_names[img_inds_org])

            per_img_verts_list = [outputs['verts'][inds].detach() for inds in per_img_inds]
            trans = [outputs['cam_trans'][inds].detach() for inds in per_img_inds]
            cam = [outputs['params']['cam'][inds].detach() for inds in per_img_inds]
            # rendered_imgs = self.visualize_renderer_verts_list(per_img_verts_list, images=org_imgs.copy(), trans=trans, colors=mesh_colors, org_res=org_res)
            rendered_imgs = self.visualize_renderer_verts_list(per_img_verts_list, images=org_img.copy(), trans=None, colors=mesh_colors, org_res=org_res, cam=cam)
    ...
    def save_rendered_img(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, save2html=False, **kwargs):
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'mesh' and detection_flag:
                ...
                index = int(os.path.basename(meta_data['imgpath'][0]).split('.')[0])
                cv2.imwrite("/home/dev2/Drive_C/MK/IdHMR/Ab02/result/3dpw_pc/{0:06d}_mesh.png".format(index), cv2.cvtColor(rendered_imgs[0], cv2.COLOR_RGB2BGR))


    def save_rendered_img_crowdpose(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, index=0, subject_ids=None, save2html=False, **kwargs):
        vis_cfg = dict(default_cfg, **vis_cfg)
        if vis_cfg['save_dir'] is None:
            vis_cfg['save_dir'] = self.result_img_dir
        os.makedirs(vis_cfg['save_dir'], exist_ok=True)
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'mesh' and detection_flag:
                # rendered_imgs = self.show_verts_on_imgs(outputs, meta_data, (used_org_inds, per_img_inds, img_inds_org), \
                #     org_imgs, img_names=img_names, put_org='put_org' in vis_cfg['settings'], plot_dict=plot_dict, save2html=save2html)
                if subject_ids[0][4] != -1:
                    index = int(os.path.basename(meta_data['imgpath'][0]).split('.')[0])
                    # cv2.imwrite("/home/dcvl/MK/IdHMR/Ab02/results/crowdpose/{0:06d}_mesh.png".format(index), cv2.cvtColor(rendered_imgs[0], cv2.COLOR_RGB2BGR))
                    cv2.imwrite("/home/dcvl/MK/IdHMR/Ab02/results/crowdpose/{0:06d}_input.png".format(index), cv2.cvtColor(outputs['meta_data']['image'][0].cpu().numpy(), cv2.COLOR_RGB2BGR))

                if subject_ids[1][4] != -1:
                    index = int(os.path.basename(meta_data['imgpath'][1]).split('.')[0])
                    # cv2.imwrite("/home/dcvl/MK/IdHMR/Ab02/results/crowdpose/{0:06d}_mesh.png".format(index), cv2.cvtColor(rendered_imgs[0], cv2.COLOR_RGB2BGR))
                    cv2.imwrite("/home/dcvl/MK/IdHMR/Ab02/results/crowdpose/{0:06d}_input.png".format(index), cv2.cvtColor(outputs['meta_data']['image'][1].cpu().numpy(), cv2.COLOR_RGB2BGR))
#################################################################################################################################
